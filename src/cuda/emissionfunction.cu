...
__global__ void blockReduction(double* dN_pTdpTdphidy_d, int final_spectrum_size, int cooperfryeblocks)
{
...
void EmissionFunctionArray::calculate_spectra()
{
  ...
  int cooperfryeblocks = (FO_length + threadsPerBlock - 1) / threadsPerBlock; //?? number of blocks in the first kernel (thread reduction)
  ...
  int blocks = (final_spectrum_size + threadsPerBlock -1) / threadsPerBlock; //?? number of blocks in the second kernel (block reduction)
  ...
  cout << "Declaring and filling host arrays with particle and freezeout info" << endl;
  //particle info
  ...
  if(debug) cout << "Starting First Kernel : Calculation of Cooper Frye integrand and thread reduction" << endl;
  ...
  calculate_dN_pTdpTdphidy<<<cooperfryeblocks, threadsPerBlock>>>(FO_length, number_of_chosen_particles, pT_tab_length, phi_tab_length, y_tab_length,
  ...
  cudaDeviceSynchronize();
  blockReduction<<<blocks, threadsPerBlock>>>(dN_pTdpTdphidy_d, final_spectrum_size, cooperfryeblocks);
  cudaDeviceSynchronize();
