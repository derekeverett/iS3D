...
#define THREADS_PER_BLOCK 128 // threads per block
#define BLOCKS 128            // number of blocks
#define FO_CHUNK 16384        // surface chunk size = threadsPerBlock x blocks
...
__global__ void calculate_dN_pTdpTdphidy_threadReduction(double* dN_pTdpTdphidy_d_blocks, long endFO, double mT, double px, double py, double y, long eta_tab_length, double* etaValues_d, double* etaWeights_d, double mass_squared, double sign, double prefactor, double baryon, double *T_d, double *P_d, double *E_d, double *tau_d, double *eta_d, double *ux_d, double *uy_d, double *un_d, double *dat_d, double *dax_d, double *day_d, double *dan_d, double *pixx_d, double *pixy_d, double *pixn_d, double *piyy_d, double *piyn_d, double *bulkPi_d, double *alphaB_d, double *nB_d, double *Vx_d, double *Vy_d, double *Vn_d, deltaf_coefficients *df_coeff_d, int INCLUDE_BARYON, int REGULATE_DELTAF, int INCLUDE_SHEAR_DELTAF, int INCLUDE_BULK_DELTAF, int INCLUDE_BARYONDIFF_DELTAF, int DIMENSION, int OUTFLOW, int DF_MODE)
{
  ...
  if(icell < endFO)
  {
    ...
    double mT_over_tau = mT / tau;     // useful expressions
    ...
    if(ithread == 0)
    {
      long iblock = blockIdx.x;

      dN_pTdpTdphidy_d_blocks[iblock] = dN_pTdpTdphidy_thread[0];
    }
...
__global__ void calculate_dN_pTdpTdphidy_blockReduction(double *dN_pTdpTdphidy_d, double* dN_pTdpTdphidy_d_blocks, long iP)
{
  long iblock = threadIdx.x;  // index of block from previous launch

  __shared__ double dN_pTdpTdphidy_block[BLOCKS];

  dN_pTdpTdphidy_block[iblock] = dN_pTdpTdphidy_d_blocks[iblock];
  ...
  while(N != 1)
  {
    ...
    if(iblock < N) // reduce thread pairs
    {
      dN_pTdpTdphidy_block[iblock] += dN_pTdpTdphidy_block[iblock + N];
    }
    __syncthreads();
  }

  // store chunk contribution to dN_pTdpTdphidy_d[iP]
  if(iblock == 0)
  {
    dN_pTdpTdphidy_d[iP] = dN_pTdpTdphidy_block[0];
  }
}
...
void EmissionFunctionArray::calculate_spectra()
{
  ...
  cudaDeviceSynchronize();    // test synchronizing device
  ...
  double trig[2 * phi_tab_length];  // {cos(phip), sin(phip)}
  ...
  double *etaValues_d, *etaWeights_d;
  ...
  cudaMalloc((void**) &dN_pTdpTdphidy_d, momentum_length * sizeof(double));
  cudaMalloc((void**) &dN_pTdpTdphidy_d_blocks, blocks_ker1 * sizeof(double));
  ...
  if(err != cudaSuccess)
  {
    printf("Error in initializing device memory: %s\n", cudaGetErrorString(err));
    err = cudaSuccess;
  }
  ...
  for(long n = 0; n < chunks; n++)
  {
    ...
    {
      cudaMemset(dN_pTdpTdphidy_d, 0.0, momentum_length * sizeof(double));
      cudaMemset(dN_pTdpTdphidy_d_blocks, 0.0, blocks_ker1 * sizeof(double));
      ...
      for(int ipT = 0; ipT < pT_tab_length; ipT++)
      {
        ...
        double mT = sqrt(pT * pT  +  mass_squared);

        for(int iphip = 0; iphip < phi_tab_length; iphip++)
        {
          double px = pT * trig[iphip];
          double py = pT * trig[iphip + phi_tab_length];
          ...
          for(int iy = 0; iy < y_tab_length; iy++)
          {
            ...
            cudaDeviceSynchronize();
            calculate_dN_pTdpTdphidy_blockReduction<<<1, blocks_ker1>>>(dN_pTdpTdphidy_d, dN_pTdpTdphidy_d_blocks, iP);
            cudaDeviceSynchronize();
      ...
      cudaMemcpy(dN_pTdpTdphidy_momentum, dN_pTdpTdphidy_d, momentum_length * sizeof(double), cudaMemcpyDeviceToHost);
