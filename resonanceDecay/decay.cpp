...
double dnpir2N (double phi, void *para1)
{
  ...
  cphiR = - jac * (para->p0 * para->sinth * cos (phi + para->phi) - para->pt * cos (para->phi)) / (sume * ptR);
  sphiR = - jac * (para->p0 * para->sinth * sin (phi + para->phi) - para->pt * sin (para->phi)) / (sume * ptR);
    ...
    case 2: // 2-body decay
    {
      ...
      for (l = 0; l < npt; l++)
      {
        for (i = 0; i < nphi; i++)
        {
          ...
          particle[pn].dNdptdphi[l][i] += particleDecay[j].branch * Edndp3_2bodyN(y, particle[pn].pt[l], PHI[i], m1, m2, mr, particle[pnR].monval);
        }
      }
      break;
    }
    ...
    case 3: //3-body decay
    {
      ...
      for (i = 0; i < nphi; i++)
      {
        for (l = 0; l < npt; l++)
        {
          ...
          particle[pn].dNdptdphi[l][i] += particleDecay[j].branch * Edndp3_3bodyN (y, particle[pn].pt[l], PHI[i], m1, m2, m3, mr, norm3, particle[pnR].monval);
        }
      }
      break;
    }
...
// int bound; monte-carlo value of a lower bound in the spectrum of particles taken into account
{
  ...
  printf ("Inside calc_reso_decays function \n");
